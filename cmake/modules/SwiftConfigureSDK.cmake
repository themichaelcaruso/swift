...
function(_report_sdk prefix)
  ...
  elseif("${prefix}" STREQUAL "ANDROID")
    ...
    foreach(arch ${SWIFT_SDK_${prefix}_ARCHITECTURES})
      ...
      message(STATUS "  ${arch} LIB: ${${arch}_LIB}")
    endforeach()
  elseif("${prefix}" STREQUAL "VEXOS")
    message(STATUS " NDK Dir: $ENV{SWIFT_VEX_SDK_PATH}")
    foreach(arch ${SWIFT_SDK_${prefix}_ARCHITECTURES})
      swift_vexos_include_for_arch(${arch} ${arch}_INCLUDE)
      swift_vexos_lib_for_arch(${arch} ${arch}_LIB)
      message(STATUS "  ${arch} INCLUDE: ${${arch}_INCLUDE}")
      message(STATUS "  ${arch} LIB: ${${arch}_LIB}")
    endforeach()
  else()
...
macro(configure_sdk_unix name architectures)
  ...
  foreach(arch ${architectures})
    ...
    elseif("${prefix}" STREQUAL "VEXOS")
          set(SWIFT_SDK_VEXOS_ARCH_${arch}_TRIPLE "thumbv7-none-eabi")
    else()
